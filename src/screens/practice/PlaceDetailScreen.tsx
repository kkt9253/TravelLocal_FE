...
const PaymentScreen = () => {
  ...
  // 가격 계산 로직 (가격이 없으면 기본 50,000원 설정)
        ...
        <View style={styles.box}>
          ...
          <View style={styles.row}>
            <Picker selectedValue={year} onValueChange={setYear} style={styles.picker}>
              {[2025, 2026].map(y => <Picker.Item key={y} label={`${y}년`} value={y} color="#000" />)}
            </Picker>
            <Picker selectedValue={month} onValueChange={setMonth} style={styles.picker}>
              {[...Array(12)].map((_, i) => <Picker.Item key={i + 1} label={`${i + 1}월`} value={i + 1} color="#000" />)}
            </Picker>
            <Picker selectedValue={day} onValueChange={setDay} style={styles.picker}>
              {[...Array(31)].map((_, i) => <Picker.Item key={i + 1} label={`${i + 1}일`} value={i + 1} color="#000" />)}
            </Picker>
          </View>
        </View>
